<script setup>
import { RegisterFormSchema } from "~/schemas/RegisterForm";
import { useField, useForm } from "vee-validate";
import { toast } from "vue3-toastify";

const isLoading = ref(false);
const { handleSubmit, errors } = useForm({
  validationSchema: RegisterFormSchema,
});

const { value: email } = useField("email");
const { value: username } = useField("username");
const { value: password } = useField("password");
const { value: confirmPassword } = useField("confirmPassword");

const onSubmit = handleSubmit(async (values) => {
  try {
    isLoading.value = true;
    await useFetch("/api/auth/register", {
      method: "POST",
      body: values,
    });

    useRouter().push({
      name: "sign-in",
    });
  } catch (err) {
    toast.error("Something went wrong");
  } finally {
    isLoading.value = false;
  }
});
</script>

<template>
  <div class="card w-full md:w-3/4 p-8 bg-ground-100">
    <h1 class="text-left py-8 font-extrabold text-lg md:text-xl">
      Sign up to WORDLES
    </h1>
    <form @submit="onSubmit" class="flex flex-col gap-y-5 md:gap-y-7 pt-8 pb-8">
      <label class="form-control w-full">
        <label
          :class="{
            'input input-bordered flex items-center gap-2': true,
            'input-error': errors.email,
          }"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            class="w-4 h-4 opacity-70"
          >
            <path
              d="M2.5 3A1.5 1.5 0 0 0 1 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.652 0l6.598-3.185A.755.755 0 0 1 15 5.293V4.5A1.5 1.5 0 0 0 13.5 3h-11Z"
            />
            <path
              d="M15 6.954 8.978 9.86a2.25 2.25 0 0 1-1.956 0L1 6.954V11.5A1.5 1.5 0 0 0 2.5 13h11a1.5 1.5 0 0 0 1.5-1.5V6.954Z"
            />
          </svg>
          <input
            type="text"
            name="email"
            id="email"
            class="grow"
            placeholder="Email"
            v-model="email"
          />
          <div class="label flex-col hidden md:block">
            <span class="label-text-alt text-red-500">{{ errors.email }}</span>
          </div>
        </label>
        <div class="items-start block md:hidden">
          <span class="label-text-alt text-red-500">{{ errors.email }}</span>
        </div>
      </label>
      <label class="form-control w-full">
        <label
          :class="{
            'input input-bordered flex items-center gap-2': true,
            'input-error': errors.username,
          }"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            class="w-4 h-4 opacity-70"
          >
            <path
              d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM12.735 14c.618 0 1.093-.561.872-1.139a6.002 6.002 0 0 0-11.215 0c-.22.578.254 1.139.872 1.139h9.47Z"
            />
          </svg>
          <input
            type="text"
            name="username"
            id="username"
            class="grow"
            placeholder="Username"
            v-model="username"
          />
          <div class="label flex-col hidden md:block">
            <span class="label-text-alt text-red-500">{{
              errors.username
            }}</span>
          </div>
        </label>
        <div class="items-start block md:hidden">
          <span class="label-text-alt text-red-500">{{ errors.username }}</span>
        </div>
      </label>
      <label class="form-control w-full">
        <label
          :class="{
            'input input-bordered flex items-center gap-2': true,
            'input-error': errors.password,
          }"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            class="w-4 h-4 opacity-70"
          >
            <path
              fill-rule="evenodd"
              d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z"
              clip-rule="evenodd"
            />
          </svg>
          <input
            autocomplete
            type="password"
            name="password"
            id="password"
            class="grow"
            placeholder="Password"
            v-model="password"
          />
          <div class="label flex-col hidden md:block">
            <span class="label-text-alt text-red-500">{{
              errors.password
            }}</span>
          </div>
        </label>
        <div class="items-start block md:hidden">
          <span class="label-text-alt text-red-500">{{ errors.password }}</span>
        </div>
      </label>
      <label class="form-control w-full">
        <label
          :class="{
            'input input-bordered flex items-center gap-2': true,
            'input-error': errors.confirmPassword,
          }"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            class="w-4 h-4 opacity-70"
          >
            <path
              fill-rule="evenodd"
              d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z"
              clip-rule="evenodd"
            />
          </svg>
          <input
            autocomplete
            type="password"
            name="confirmPassword"
            id="confirmPassword"
            class="grow"
            placeholder="Confirm Password"
            v-model="confirmPassword"
          />
          <div class="label flex-col hidden md:block">
            <span class="label-text-alt text-red-500">{{
              errors.confirmPassword
            }}</span>
          </div>
        </label>
        <div class="items-start block md:hidden">
          <span class="label-text-alt text-red-500">{{
            errors.confirmPassword
          }}</span>
        </div>
      </label>
      <div
        class="flex flex-col-reverse md:flex-row justify-between items-start gap-y-2 md:items-center"
      >
        <button
          :disabled="isLoading"
          class="btn bg-indigo-500 hover:bg-indigo-600 text-white"
        >
          Sign Up <span class="loading loading-spinner" v-if="isLoading"></span>
        </button>
        <span class="flex justify-end items-end w-full gap-x-2 text-sm"
          >Have an account already ?
          <NuxtLink class="text-indigo-500" to="/sign-in"
            >Sign In</NuxtLink
          ></span
        >
      </div>
    </form>
  </div>
</template>
